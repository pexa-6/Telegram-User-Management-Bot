...
def ensure_db():
    ...
    Если таблица уже есть, но в ней не хватает новых колонок — добавляет их.
    Структура таблицы:
    ...
    with sqlite3.connect(DB_PATH) as db:
        ...
        if "email" not in columns:
            cursor.execute("ALTER TABLE list ADD COLUMN email TEXT")
        if "instagram" not in columns:
            cursor.execute("ALTER TABLE list ADD COLUMN instagram TEXT")
        if "tiktok" not in columns:
            cursor.execute("ALTER TABLE list ADD COLUMN tiktok TEXT")
...
def insert_data(data):
    ...
    data — кортеж (user_id, user, name, tag, phone, email, instagram, tiktok)
    Используется при автоматическом логировании входящих сообщений и при /add.
    """
    with sqlite3.connect(DB_PATH) as db:
        cursor = db.cursor()
        cursor.execute(
            "INSERT OR REPLACE INTO list(user_id, user, name, tag, phone, email, instagram, tiktok) VALUES(?, ?, ?, ?, ?, ?, ?, ?)",
            data
        )
        db.commit()
...
def send_welcome(message):
    ...
    bot.reply_to(message, "Выбери одну из команд:\n"
                          ...
                          "/replace_email – Заменить Email по Number.\n"
                          "/replace_instagram – Заменить Instagram по Number.\n"
                          "/replace_tiktok – Заменить Tiktok по Number.\n"
...
def write_phone(message, user_id, user, name, tag):
    ...
    if is_skip or is_phone:
        ...
        bot.send_message(message.chat.id, "Введите Email или - если нет:")
        bot.register_next_step_handler(message, write_email, user_id, user, name, tag, phone)
        return
...
def write_email(message, user_id, user, name, tag, phone):
    """Получаем email, проверяем формат или пропуск."""
    email = message.text.strip()

    is_skip = email == "-"
    is_email = "@" in email and "." in email and len(email) > 3

    if is_skip or is_email:
        if is_skip:
            email = ""

        bot.send_message(message.chat.id, "Введите Instagram или - если нет:")
        bot.register_next_step_handler(message, write_instagram, user_id, user, name, tag, phone, email)
        return

    bot.reply_to(message, "❌ Введите email в корректном формате или - если не знаете.")
    bot.register_next_step_handler(message, write_email, user_id, user, name, tag, phone)


def write_instagram(message, user_id, user, name, tag, phone, email):
    """Получаем Instagram или пропуск."""
    instagram = message.text.strip()

    if instagram == "-":
        instagram = ""

    bot.send_message(message.chat.id, "Введите Tiktok или - если нет:")
    bot.register_next_step_handler(message, write_tiktok, user_id, user, name, tag, phone, email, instagram)


def write_tiktok(message, user_id, user, name, tag, phone, email, instagram):
    """Получаем Tiktok и сохраняем запись в БД."""
    ...
    try:
        insert_data((user_id, user, name, tag, phone, email, instagram, tiktok))
...
# -----------------------
# Replace / Update handlers (replace_name / replace_user / replace_tag / replace_email / replace_instagram / replace_tiktok)
# -----------------------
...
@bot.message_handler(commands=['replace_email'])
def replace_email_cmd(message):
    """Показ пагинации и запрос Number для замены email."""
    send_paginated_list(message.chat.id)
    bot.send_message(message.chat.id, "Введите Number кому хотите заменить Email:")
    bot.register_next_step_handler(message, setting_replace_email)


def setting_replace_email(message):
    """Проверка Number и запрос нового email."""
    id = message.text.strip()
    ...
    if result:
        bot.send_message(message.chat.id, "Введите новый Email или - если удалить:")
        bot.register_next_step_handler(message, replace_email, id)
    else:
        bot.reply_to(message, "❔ Такого Number нет.\n/replace_email")


def replace_email(message, id):
    """Обновление/удаление поля email."""
    email = message.text.strip()
    if email == "-":
        email = ""
    ...
    try:
        with sqlite3.connect(DB_PATH) as db:
            cursor = db.cursor()
            cursor.execute("UPDATE list SET email = ? WHERE id = ?", (email, id))
            db.commit()

        bot.send_message(message.chat.id, f"✅ Email обновлён для Number {id}", reply_markup=start)
    except Exception as e:
        error_logger.error(f"Ошибка в работе бота через: {getattr(message.from_user,'id','unknown')}\nError: {e}\n", exc_info=True)
        bot.reply_to(message, f"❌ Ошибка\n/replace_email")


@bot.message_handler(commands=['replace_instagram'])
def replace_instagram_cmd(message):
    """Показ пагинации и запрос Number для замены instagram."""
    send_paginated_list(message.chat.id)
    bot.send_message(message.chat.id, "Введите Number кому хотите заменить Instagram:")
    bot.register_next_step_handler(message, setting_replace_instagram)


def setting_replace_instagram(message):
    """Проверка Number и запрос нового instagram."""
    id = message.text.strip()
    ...
    if result:
        bot.send_message(message.chat.id, "Введите новый Instagram или - если удалить:")
        bot.register_next_step_handler(message, replace_instagram, id)
    else:
        bot.reply_to(message, "❔ Такого Number нет.\n/replace_instagram")


def replace_instagram(message, id):
    """Обновление/удаление поля instagram."""
    instagram = message.text.strip()
    if instagram == "-":
        instagram = ""
    ...
    try:
        with sqlite3.connect(DB_PATH) as db:
            cursor = db.cursor()
            cursor.execute("UPDATE list SET instagram = ? WHERE id = ?", (instagram, id))
            db.commit()

        bot.send_message(message.chat.id, f"✅ Instagram обновлён для Number {id}", reply_markup=start)
    except Exception as e:
        error_logger.error(f"Ошибка в работе бота через: {getattr(message.from_user,'id','unknown')}\nError: {e}\n", exc_info=True)
        bot.reply_to(message, f"❌ Ошибка\n/replace_instagram")


@bot.message_handler(commands=['replace_tiktok'])
def replace_tiktok_cmd(message):
    """Показ пагинации и запрос Number для замены tiktok."""
    send_paginated_list(message.chat.id)
    bot.send_message(message.chat.id, "Введите Number кому хотите заменить Tiktok:")
    bot.register_next_step_handler(message, setting_replace_tiktok)


def setting_replace_tiktok(message):
    """Проверка Number и запрос нового tiktok."""
    id = message.text.strip()
    ...
    if result:
        bot.send_message(message.chat.id, "Введите новый Tiktok или - если удалить:")
        bot.register_next_step_handler(message, replace_tiktok, id)
    else:
        bot.reply_to(message, "❔ Такого Number нет.\n/replace_tiktok")


def replace_tiktok(message, id):
    """Обновление/удаление поля tiktok."""
    tiktok = message.text.strip()
    if tiktok == "-":
        tiktok = ""
    ...
    try:
        with sqlite3.connect(DB_PATH) as db:
            cursor = db.cursor()
            cursor.execute("UPDATE list SET tiktok = ? WHERE id = ?", (tiktok, id))
            db.commit()

        bot.send_message(message.chat.id, f"✅ Tiktok обновлён для Number {id}", reply_markup=start)
    except Exception as e:
        error_logger.error(f"Ошибка в работе бота через: {getattr(message.from_user,'id','unknown')}\nError: {e}\n", exc_info=True)
        bot.reply_to(message, f"❌ Ошибка\n/replace_tiktok")
